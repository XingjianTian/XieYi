...
public class Water : MonoBehaviour {
	...
	public waterPCtest hero_wpc;
	public Rigidbody2D boat_rig;
    ...
    //All our constants
    const float springconstant = 0.018f;
    ...
    public void Splash(float xpos, float velocity)
    {
        ...
        if (xpos >= xpositions[0] && xpos <= xpositions[xpositions.Length-1])
        {
            ...
            float lifetime = 0.1f + Mathf.Abs(velocity)*0.07f;

            //Set the splash to be between two values in Shuriken by setting it twice.
            splash.GetComponent<ParticleSystem>().startSpeed = (2 + 2 * Mathf.Pow(Mathf.Abs(velocity), 0.5f))/2;
            splash.GetComponent<ParticleSystem>().startSpeed = (3 + 2 * Mathf.Pow(Mathf.Abs(velocity), 0.5f))/2;
            splash.GetComponent<ParticleSystem>().startLifetime = lifetime;

            //Set the correct position of the particle system.
            Vector3 position = new Vector3(xpositions[index],ypositions[index]-0.05f,5);

            //This line aims the splash towards the middle. Only use for small bodies of water:
			Quaternion rotation = Quaternion.LookRotation(new Vector3(position.x - boat_rig.gameObject.transform.position.x, baseheight + 8 - position.y, 5 - position.z));
    ...
    public void SpawnWater(float Left, float Width, float Top, float Bottom)
    {
		...
		box.offset = new Vector2(Left + Width / 2, (Top + Bottom) / 2);
		box.size = new Vector2(Width, Top - Bottom);
		box.isTrigger = true;
        ...
        for (int i = 0; i < edgecount; i++)
        {
            ...
            colliders[i].transform.position = new Vector3(Left + Width * (i + 0.5f) / edgecount, Top - 0.14f, 0);
            colliders[i].transform.localScale = new Vector3(Width / edgecount, 0.3f, 1);

            //Add a WaterDetector and make sure they're triggers
            colliders[i].GetComponent<BoxCollider2D>().isTrigger = true;
            colliders[i].AddComponent<WaterDetector>();

			colliders [i].layer = LayerMask.NameToLayer ("Ground");
        }
    ...
    void OnTriggerStay2D(Collider2D Hit)
    {
		...
		if (Hit.name == "Boat" && Hit.GetComponent<Rigidbody2D> () != null) {
			// print (Hit.name);
			//Rigidbody2D Hit_Rig = Hit.GetComponent<Rigidbody2D> ();
			float hx = Hit.transform.position.x, hy = Hit.transform.position.y, hv = boat_rig.velocity.y;
			int points = Mathf.RoundToInt (Hit.transform.localScale.x * 15f);

			hx -= xpositions [0];
			int index = Mathf.RoundToInt ((xpositions.Length - 1) * (hx / (xpositions [xpositions.Length - 1] - xpositions [0])));
			if (index > 0 && index < xpositions.Length) {
				//print(index);
				float force = springconstant * (hy - ypositions [index] - 0.15f) + hv * damping;
				float ha = -force;
				boat_rig.AddTorque (150 * (rightDeltas [index] - leftDeltas [index]), ForceMode2D.Force);
				boat_rig.AddForce (new Vector2 (0, Mathf.Min (110.0f * (ypositions [index] + 0.15f - hy), boat_rig.mass * 12) - hv * 1));
			}
		} 
		else if (Hit.name == "NewHero") {
			hero_wpc.inWater = true;
		}
    }
